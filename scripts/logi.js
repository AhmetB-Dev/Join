(() => {
  ...
  const EMAIL_REGEX = /^[^\s@]+@([A-Za-z0-9-]+\.)+[A-Za-z]{10}$/;

  function getMessageElement() {
    const mode =
      (typeof window !== "undefined" && window.mode) ||
      (document.getElementById("signupPanel")?.classList.contains("show")
        ? "signup"
        : "login");
  ...
  function applyInvalidState(...elements) {
    elements
      .filter(Boolean)
      .forEach((node) => node.setAttribute("aria-invalid", "true"));
  }
  ...
  async function fetchUserByEmail(lowerEmail) {
    ...
    try {
      ...
      if (indexedId)
        return await window.firebaseGet(
          `/users/${encodeURIComponent(indexedId)}`
        );
    } catch {}
    try {
  ...
  function validateSignupPayload(payload) {
    ...
    if (payload.password !== payload.passwordConfirm) {
      return {
        ...
        fields: [signupPasswordInput, signupPasswordConfirmInput],
      };
    }
    return { ok: true };
  }
    ...
    try {
      ...
      setTimeout(() => {
        if (
          ...
          typeof window.renderAuthUI === "function"
        ) {
          window.mode = "login";
          window.renderAuthUI();
        }
